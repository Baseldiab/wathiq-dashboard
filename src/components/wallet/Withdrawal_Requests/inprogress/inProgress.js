...
export default function InProgress() {
    ...
    dispatch(getOneWithDraw(id))
      ...
      .catch(() => {
        setAlert({
          type: "error",
          msg: t(
            "components_wallet_withdrawal-requests_inprogress_inProgress.حدث_خطأ_أثناء_تحميل_البيانات"
          ),
        });
        setAlertOpen(true);
      });
  ...
  useEffect(() => {
    dispatch(
      getWithDraw(buildQueryParams({ status: "inProgress", page, limit: 5 }))
    );
  }, [dispatch]); // أول تحميل
  ...
  const handlePaginationChange = (_e, currentPage) => {
    ...
    dispatch(
      getWithDraw(
        buildQueryParams({ status: "inProgress", page: currentPage, limit: 5 })
      )
    );
  };
          ...
          {Data?.data?.length > 0 ? (
            Data.data.map((item) => (
              <TransactionCard
                ...
                title={t(
                  "components_wallet_withdrawal-requests_inprogress_inProgress.طلب_سحب_رصيد"
                )}
              />
            ))
          ) : (
            <NoData
              ...
              desc={t(
                "components_wallet_withdrawal-requests_inprogress_inProgress.لا_توجد_معاملات"
              )}
            />
          )}
        </Box>
        ...
        {Data?.data?.length > 0 && (
          ...
          >
            ...
            >
              ...
              >
                {t(
                  "components_wallet_withdrawal-requests_inprogress_inProgress.من"
                )}
              </Typography>
              <Typography
            ...
            dispatch(updateOneWithDraw({ id, data: { status: newStatus } }))
              ...
              .then(() => {
                ...
                setAlert({
                  type: "success",
                  msg: t(
                    "components_wallet_withdrawal-requests_inprogress_inProgress.تم_تحديث_حالة_الطلب"
                  ),
                });
                setAlertOpen(true);
                dispatch(
                  getWithDraw(
                    buildQueryParams({ status: "inProgress", page, limit: 5 })
                  )
                );
              })
              .catch(() => {
                setAlert({
                  type: "error",
                  msg: t(
                    "components_wallet_withdrawal-requests_inprogress_inProgress.فشل_تحديث_حالة_الطلب"
                  ),
                });
                setAlertOpen(true);
              });
