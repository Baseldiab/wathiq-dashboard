...
export default function Rejected() {
  ...
  const handleOpen = (id) => {
    ...
    try {
      ...
      setAlert({ type: "error", msg: t("components_wallet_rejected.حدث_خطأ_أثناء_تحميل_البيانات") });
      setAlertOpen(true);
    }
  };
  ...
  useEffect(() => {
    dispatch(getWithDraw(buildQueryParams({ status: "rejected", page,limit:5 })));
  }, [dispatch]);
  ...
  const handlePaginationChange = (e, currentPage) => {
    setPage(currentPage);
    dispatch(getWithDraw(buildQueryParams({ status: "rejected", page: currentPage,limit:5 })));
  };
        ...
        {Data?.data?.length > 0 ? (
          Data.data.map((item) => (
            <TransactionCard
              ...
              title={t("components_wallet_rejected.طلب_سحب_رصيد")}
            />
          ))
        ) : (
          <NoData img="/images/no-wallet.svg" desc={t("components_wallet_rejected.لا_توجد_معاملات")} />
        )}
      </Box>
        ...
        >
          <Box sx={{ display: "flex", gap: "12px" }}>
            <Typography sx={{ color: "#6F6F6F", fontWeight: 700, fontSize: "1rem" }}>
              {t("components_wallet_rejected.من")}
            </Typography>
      ...
      <WithdrawDetailsForm
        ...
        onConfirm={(newStatus) => {
          ...
          dispatch(updateOneWithDraw({ id, data: { status: newStatus } }))
            ...
            .then(() => {
              handleClose();
              setAlert({ type: "success", msg: t("components_wallet_rejected.تم_تحديث_حالة_الطلب") });
              setAlertOpen(true);
              dispatch(getWithDraw(buildQueryParams({ status: "rejected", page,limit:5 })));
            })
            .catch((err) => {
              console.error("Update failed", err);
              setAlert({ type: "error", msg: t("components_wallet_rejected.فشل_تحديث_حالة_الطلب") });
              setAlertOpen(true);
            });
        }}
      />
